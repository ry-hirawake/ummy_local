    ...
    >
      ...
      <div className="flex-1">
        ...
        <div className="mt-1 flex items-center gap-4 px-3 text-xs text-muted-foreground">
          <button className="hover:text-primary transition-colors">いいね</button>
          <button 
                      ...
                      {Object.entries(post.reactions).map(([type, count]) => {
                        if (count > 0) {
                          ...
                          return (
                            ...
                            >
                              <ReactionIcon className={`h-3 w-3 ${reactionIcons[type as keyof typeof reactionIcons].color}`} />
                            </div>
                          );
                        }
                        return null;
                ...
                <div className="relative flex-1">
                  ...
                  >
                    {post.userReaction ? (
                      <>
                        {(() => {
                          const ReactionIcon = reactionIcons[post.userReaction as keyof typeof reactionIcons].icon;
                          return <ReactionIcon className="h-4 w-4" />;
                        })()}
                        <span className="font-medium">
                          {reactionIcons[post.userReaction as keyof typeof reactionIcons].label}
                        </span>
                      </>
                    ) : (
                      <>
